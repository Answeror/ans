...
class pimpl<T>::detail : pimpl_base_
{
    ...
    //1>d:\code\ans\ans\pimpl\pimpl.h(89) : fatal error C1001: An internal error has occurred in the compiler.
   ...
   ~detail () {} // Defined explicitly as 'protected'.
    ...
    void reset(
        Impl* impl, 
        typename boost::enable_if<boost::is_same<Impl, implementation> >::type* = 0
    ) { impl_.reset(impl); }
    ...
    detail(
        UseDefaultCtor, 
        typename boost::enable_if<boost::is_same<UseDefaultCtor, use_default_ctor> >::type* = 0
